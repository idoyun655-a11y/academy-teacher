...
qs("#bulkSaveBtn")?.addEventListener("click", async ()=>{
  try{
    ...
    const title = (qs("#title")?.value || "").trim();
    const status = qs("#status").value;
    if(!title) return toast("숙제 제목 입력!");
    const targets = (studentsCache||[]).filter(s => (s.class_name||"").trim() === class_name);
    if(!targets.length) return toast("해당 반 학생이 없어!");
    const rows = targets.map(s => ({ student_id: s.id, due_date, title, status }));
    const { error } = await supabase.from("homework").upsert(rows, { onConflict: "student_id,due_date,title" });
    if(error) throw error;
